...
async def find_doctors(lat: float = Query(None), lon: float = Query(None),
                       city: str = Query(None), radius: int = 10000,
                       type: str = Query(None)):
    """Find doctors/clinics near coordinates or city. Optionally filter by specialty type."""
    if city and (lat is None or lon is None):
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                geo_res = await client.get(NOMINATIM_URL, params={
                ...
                if res.status_code == 200:
                    ...
                    for e in elements:
                        ...
                        results.append({
                            "id": e["id"],
                            "name": name,
                            "type": e.get("tags", {}).get("amenity") or e.get("tags", {}).get("healthcare") or "clinic",
                            "phone": e.get("tags", {}).get("phone") or e.get("tags", {}).get("contact:phone"),
                            "lat": e_lat, "lon": e_lon, "dist": dist,
                        })
                    results.sort(key=lambda x: x["dist"])
                    return {
                        "elements": results[:20],
